...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "rubric criterion using a number from 0.0 to 1.0. Return only a "
            "JSON object mapping criterion names to scores.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        object_match = re.search(r"\{.*\}", raw_response, flags=re.DOTALL)
        if object_match:
        ...
        if isinstance(parsed, dict):
            ...
            score_source = nested_scores if isinstance(nested_scores, dict) else parsed
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            recorded_id = metadata.get("id") if isinstance(metadata, dict) else None
            failure_id = recorded_id or f"F{index:03d}"
            failure_type = failure.failure_type or "unknown"
            root_cause = self.find_root_cause(failure)
            suggestion = (
                suggestions[index - 1]
                if index <= len(suggestions)
                else "Review the trace and implement a targeted corrective action"
            )
            lines.append(
                f"| {escape(failure_id)} | {escape(failure_type)} | "
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": (
                "Add a grounding check that rejects or rewrites claims unsupported "
                "by retrieved context"
            ),
            "irrelevant": (
                "Improve intent routing and prompt clarity so answers address the "
                "user's question directly"
            ),
            "incomplete": (
                "Improve retrieval coverage and require generation to cover every "
                "condition in the expected answer"
            ),
            "off_topic": (
                "Add intent classification and domain routing before answer generation"
            ),
            "refusal": (
                "Calibrate guardrails with allowed in-scope examples to reduce "
                "unnecessary refusals"
            ),
            "unknown": (
                "Inspect answer and retrieval traces, then assign a concrete failure "
                "taxonomy before remediation"
            ),
        }

        suggestions: list[str] = []
        ordered_types = sorted(categories, key=lambda key: -categories[key])
        for raw_type in ordered_types:
            normalized = raw_type.lower()
            if "hallucination" in normalized:
                category = "hallucination"
            elif "irrelevant" in normalized:
                category = "irrelevant"
            elif "incomplete" in normalized or "completeness" in normalized:
                category = "incomplete"
            elif "off_topic" in normalized or "off topic" in normalized:
                category = "off_topic"
            elif "refusal" in normalized:
                category = "refusal"
            else:
                category = "unknown"
            suggestion = suggestion_by_type[category]
            if suggestion not in suggestions:
                suggestions.append(suggestion)

        general_suggestions = [
            "Add failed cases to the regression benchmark and enforce metric quality gates",
            "Review retrieved chunks alongside answers to separate retrieval from generation failures",
            "Calibrate automatic scores against human labels before changing deployment thresholds",
        ]
